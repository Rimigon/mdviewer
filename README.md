...
- **Правка / Просмотр** — переключатель режима: split-редактор с живым превью или чистый рендер
...
- **Ссылки наружу** — внешние ссылки открываются в браузере по умолчанию, окно приложения не покидает
